...
#         model.eval()
    ...
    if(params['logging']!='neptune' or params['is_model'] == True):
        ...
        # Neptune logging disabled - update to neptune 1.x API if needed
        print("Neptune logging is disabled. Set params['logging']='local' for local logging.")
        print(" Accuracy: {0:.2f}".format(testacc))
        print(" Fscore: {0:.2f}".format(testf1))
...
#         print(y_test)
        ...
        params['weights']=compute_class_weight('balanced', classes=np.unique(y_test), y=y_test).astype('float32') 
    ...
    if(params['logging']=='neptune'):
        # Neptune logging disabled - use local logging instead
        print(f"Starting experiment: {name_one}")
        print("Note: Neptune logging is disabled. Using local logging.")
        params['logging'] = 'local'  # Force local logging
    ...
    for epoch_i in range(0, params['epochs']):
        ...
        for step, batch in tqdm(enumerate(train_dataloader)):
            ...
            # Batch loss logging (neptune disabled)
        ...
        # Log metrics locally
        print(f"  Test - fscore: {test_fscore:.4f}, accuracy: {test_accuracy:.4f}")
        print(f"  Val  - fscore: {val_fscore:.4f}, accuracy: {val_accuracy:.4f}")
        print(f"  Train- fscore: {train_fscore:.4f}, accuracy: {train_accuracy:.4f}")
    ...
    # Print best results (local logging)
...
if __name__=='__main__': 
    ...
    ##### change in logging to output the results locally (neptune disabled)
    params['logging']='local'
    # Neptune logging disabled - to enable, install neptune and update API
    # if(params['logging']=='neptune'):
    #     import neptune
